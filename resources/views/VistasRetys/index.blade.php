@extends('VistasRetys.plantilla')
@section('tarjetas')



<!-- Categorias -->



<section class="pt-4 pb-4">
	
	<div class="container-fluid">
		
		<div class="row mb-3">
			
		
		
			<div class="col-9">		
				
				<strong>Categorias Columna 2</strong>
				<br>

				<div class="btn-group mr-2" role="group" aria-label="First group">
					
					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/cana.png"
									class="img">
								<p class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Adultos Mayores</p>
							</div>
						</div>
					</button>
					
					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_EDUCACION 2.png"
									class="img">
								<p class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Educación</p>
							</div>
						</div>
					</button>
					
					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_CULTURA 2.png"
									class="img">
								<p class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Cultura </p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_ECONOMIA Y NEGOCIO 2.png"
									class="img" >
								<p class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Finanzas</p>
							</div>
						</div>
					</button>

				</div>

				<br>

				<div class="btn-group mr-2" role="group" aria-label="Second group">

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/reclutamiento2.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Empleo</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_FAMILIA CIUDADANIA .png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Familia</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_JURIDICO .png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Justicia</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_MEDIO AMBIENTE G-45.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Medio Ambiente</p>
							</div>
						</div>
					</button>

				</div>
				
					
					
				<div class="btn-group mr-2" role="group" aria-label="Third group">
					
					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_PAGOS IMPUESTOS 2.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Pago de Impuestos</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_QUEJA DENUNCIA 2.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Quejas o Denuncias</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_SALUD 2.png"
									class="img" >
								<p
								>Salud y Deporte</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_SEGURIDAD 2.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Seguridad</p>
							</div>
						</div>
					</button>
				
				</div>	

				<div class="btn-group mr-2" role="group" aria-label="Fourd group">
				
					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_TRANSPORTE G.png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Transporte y Urbanismo</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/ICONOS_TURISMO .png"
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Turismo</p>
							</div>
						</div>
					</button>

					<button type="button" class="btn">
						<div class="card border-0 bg-transparent responsive-categorias card-body">
							<div class="responsive-categorias">
								<img
									src="img/icon/casa2.png"
<<<<<<< HEAD
									class="img-fluid" width="70">
								<p class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Vivienda</p>
=======
									class="img" >
								<p
									class="font-weight-bold text-special-color-dark align-self-center responsive-text-categorias">Vivienda</p>
>>>>>>> 3a6c07d738ea9cacaafab1a12484465430d3338d
							</div>
						</div>
					</button>
					
					
				
				</div>

			</div>	
			


			<div class="col-3">
				<strong>Columna 3</strong>
           
<<<<<<< HEAD
					<iframe width="450" height="295" src="https://www.youtube.com/embed/CrRL854defQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
=======
					<iframe width="560" height="315" src="https://www.youtube.com/embed/CrRL854defQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
>>>>>>> 3a6c07d738ea9cacaafab1a12484465430d3338d
					 <div class="icon-box">
					 
					  
					</div>

					<div class="tab">
						<ul class="nav nav-tabs">
							<li class="nav-item">
							  <a class="nav-link active" href="#">Carpeta Ciudadana</a>
							</li>
							<li class="nav-item">
							  <a class="nav-link" href="#">Seguimineto de Tramite</a>
							</li>
						
						  </ul>
					</div>
			</div>

		</div>
	</div>

</section>

<!-- Destacados -->
@stop